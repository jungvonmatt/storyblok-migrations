...
| `run [filename]`     | `-d, --dry-run`: Preview changes<br>`-s, --space <id>`: Space ID<br>`-t, --token <token>`: OAuth token<br>`-p, --publish <mode>`: Publish mode<br>`-l, --languages <langs>`: Languages to publish<br>`--throttle <ms>`: Delay between requests | Runs a schema or content migration file against Storyblok. Provide only the filename (e.g., `02-create-datasource.ts`) as the migrations are always loaded from the `migrations` directory. If no filename is provided, you can select from available migrations. |
